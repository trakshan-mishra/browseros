...
export default function SettingsApp() {
  const { 
    wallpaper, setWallpaper, volume, setVolume, 
    brightness, setBrightness, wifi, toggleWifi, 
    activeDeviceId, setActiveDevice, account, login,
    username, setUsername, devices 
  } = useOS();
  ...
  const tabs = [
    ...
    { id: 'account', name: 'Cloud Account', icon: '👤' },
    { id: 'appearance', name: 'Appearance', icon: '🎨' },
    { id: 'ai', name: 'AI Configuration', icon: '🤖' },
    { id: 'devices', name: 'Cloud Devices', icon: '☁️' },
    ...
    <div className="settings">
      ...
      <div className="settings-content">
          ...
          <div className="settings-panel">
            ...
            <div className="settings-group">
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <input className="settings-input" value={localUsername} 
                  onChange={e => setLocalUsername(e.target.value)} placeholder="Enter your name" />
                <button className="settings-save-btn" style={{ marginTop: 0 }} 
                  onClick={() => setUsername(localUsername)}>Update</button>
              </div>
            </div>
            <div className="settings-group">
        ...
        {tab === 'account' && (
          <div className="settings-panel">
            <h2>Cloud Account</h2>
            <p className="settings-desc">Log in with a username to sync your OS state across devices.</p>
            <div className="settings-group">
              <label>Account ID</label>
              <div style={{ display: 'flex', gap: '8px' }}>
                <input className="settings-input" value={localAccount} 
                  onChange={e => setLocalAccount(e.target.value)} placeholder="e.g. user_123" />
                <button className="settings-save-btn" style={{ marginTop: 0 }} 
                  onClick={() => login(localAccount)}>Login</button>
              </div>
              <small className="settings-hint">Current Account: <strong>{account}</strong></small>
            </div>
          </div>
        )}
        {tab === 'appearance' && (
            ...
            <div className="settings-group">
              ...
              <div className="provider-cards">
                <button className={`provider-card ${aiConfig.provider === 'groq' ? 'provider-active' : ''}`}
                  ...
                  <span className="provider-model">Llama 3 8B (Tool Use)</span>
                </button>
        ...
        {tab === 'devices' && (
          <div className="settings-panel">
            <h2>Cloud Devices</h2>
            <p className="settings-desc">Select a connected device to route AuraAI commands to it remotely.</p>
            <div className="settings-group">
              <label>Target Device</label>
              <div className="provider-cards" style={{ flexDirection: 'column' }}>
                <button 
                  className={`provider-card ${activeDeviceId === 'local' ? 'provider-active' : ''}`}
                  onClick={() => setActiveDevice('local')}>
                  <span className="provider-icon">💻</span>
                  <span className="provider-name">Local Machine</span>
                  <span className="provider-model">Default</span>
                </button>
                {devices.map(d => (
                  <button key={d.id} 
                    className={`provider-card ${activeDeviceId === d.id ? 'provider-active' : ''}`}
                    onClick={() => setActiveDevice(d.id)}>
                    <span className="provider-icon">📱</span>
                    <span className="provider-name">{d.name}</span>
                    <span className="provider-model">ID: {d.id}</span>
                  </button>
                ))}
              </div>
            </div>
